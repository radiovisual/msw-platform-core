...
describe('middleware helpers', () => {
	...
	it('createCustomMiddleware with feature flag context', () => {
		...
		const context = makeContext({ experimentalFeature: true }, { featureFlags: { EXPERIMENTAL_HELLO: true } });
		const result = mw.getMiddleware()(payload, context, next);
		expect(result.experimental.enabled).toBe(true);

		// Without required flag enabled
		const context2 = makeContext({ experimentalFeature: true }, { featureFlags: { EXPERIMENTAL_HELLO: false } });
	...
	it('createCustomMiddleware with status context', () => {
		...
		// With 404 status
		const context = makeContext({ errorMessage: 'Not found' }, { currentStatus: 404 });
		const result = mw.getMiddleware()(payload, context, next);
		expect(result.error.message).toBe('Not found');

		// With 200 status (no transform)
		const context2 = makeContext({ errorMessage: 'Not found' }, { currentStatus: 200 });
		const result2 = mw.getMiddleware()(payload, context2, next);
	...
	it('createCustomMiddleware with scenario context', () => {
		...
		// With guest scenario
		const context = makeContext({ scenarioOverride: 'minimal' }, { endpointScenario: 'guest' });
		const result = mw.getMiddleware()(payload, context, next);
		expect(result.user.permissions).toBe('minimal');

		// Without scenario (no transform)
		const context2 = makeContext({ scenarioOverride: 'minimal' }, { endpointScenario: undefined });
		const result2 = mw.getMiddleware()(payload, context2, next);
