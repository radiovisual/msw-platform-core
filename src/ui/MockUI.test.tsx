...
describe('MockUI', () => {
	...
	it('preserves search text across multiple UI operations (comprehensive regression test)', async () => {
		...
		const toggleButton = within(screen.getByText('/api/v1/foo').closest('div[style*="background: white"]') as HTMLElement).getByLabelText(
			/Toggle endpoint/
		);
		...
		const statusButtons = within(screen.getByText('/api/v1/foo').closest('div[style*="background: white"]') as HTMLElement).getAllByText(
			/^(200|400|503)$/
		);
		...
		const delayInput = within(screen.getByText('/api/v1/foo').closest('div[style*="background: white"]') as HTMLElement).getByDisplayValue(
			'150'
		) as HTMLInputElement;
