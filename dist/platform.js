...
class InMemoryPersistence {
    ...
    getEndpointScenario(pluginId) { return this.endpointScenarios[pluginId]; }
    setEndpointScenario(pluginId, scenarioId) { this.endpointScenarios[pluginId] = scenarioId; }
}
...
class MockPlatformCore {
    constructor(config, persistence) {
        ...
        this.name = (_a = config === null || config === void 0 ? void 0 : config.name) !== null && _a !== void 0 ? _a : new Date().getTime().toString();
        if (!this.name) {
        ...
        for (const plugin of this.plugins) {
            ...
            if (scenarioId)
                this.endpointScenarioOverrides[plugin.id] = scenarioId;
        }
    ...
    getPluginsByComponentId() {
        const map = {};
        for (const plugin of this.plugins) {
            if (!map[plugin.componentId])
                map[plugin.componentId] = [];
