...
function mswHandlersFromPlatform(platformOrGetter, options) {
    ...
    return getPlatform().getPlugins().map((plugin) => {
        const method = plugin.method.toLowerCase();
        // @ts-ignore: dynamic method access is valid
        const handler = (endpoint) => msw_1.http[method](endpoint, (req) => {
            var _a, _b, _c, _d;
                    ...
                    if (typeof urlString === 'string') {
                        ...
                        if (match) {
                            ...
                            if (qr && typeof qr === 'object' && Object.keys(qr).some(k => !isNaN(Number(k)))) {
                                // Map of status codes
                                response = (_d = (_c = qr[status]) !== null && _c !== void 0 ? _c : qr[plugin.defaultStatus]) !== null && _d !== void 0 ? _d : Object.values(qr)[0];
                            }
                            else {
                                response = qr;
                            }
